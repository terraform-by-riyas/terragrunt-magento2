...
## installation
sudo apt-get -qqy install git curl wget zip
...
export TERM=xterm
wget -c http://mirrors.linuxeye.com/oneinstack-full.tar.gz && tar xzf oneinstack-full.tar.gz && sudo ./oneinstack/install.sh --nginx_option 1 --php_option 12 --phpcache_option 1 --php_extensions imagick,fileinfo,redis,memcached
...
echo "this is admin server" > /home/ubuntu/server.txt
...
echo "this is varnish server" > /home/ubuntu/server.txt
